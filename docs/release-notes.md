...
- `lore-transport`: the process-global user agent lives at the crate root rather than in `lore_transport::grpc`, since both transports resolve the same string, and `user_agent()` / `set_user_agent()` become `user_agent_product()` / `set_user_agent_product()`, with `make_user_agent` and `make_user_agent_with_component` formatting the RFC 9110 comment. `CompressionMode` is renamed `ServerCompressionMode`
- `lore-server`: `[server.auth]` refuses to start without both `jwt_issuer` and `jwt_audience`, and `auth_url` without a `[server.auth]` block is refused too. Both previously started a server that then verified no token at all. `lock_service.max_encoding_message_size` moves under `lock_service.general`
...
- Partial support for SWFS, a virtual filesystem implementation. Currently only supports cloning a repository and is gated behind the `swfs` feature. Requires the not-yet publicly available SWFS drivers
...
- `lore branch diff` reports the branches and revisions it resolved on the `BranchDiffBegin` event, which the CLI now prints from, rather than only inside a log line
...
- `lore-server`: a replicated store's `get_metadata` and `query` take the two reserved priority QUIC streams instead of queueing behind bulk fragment traffic — p50 falls from 15.4 ms to 1.6 ms behind 128 concurrent 32 KiB gets. `ReplicatedStore` spans now measure the awaited call rather than its future's construction
